...
import { getGamificationState, recordResetCompletion } from "@/lib/gamification";
 ...
 * exception is the reset streak (`streakState`), which persists to
 * localStorage — per-browser, not per-account, since there's no login — so
 * a return visit the next day can actually extend it.
 *
 * Model:
...
export function WishlistProvider({ children }) {
  ...
  function recordReset() {
    const next = recordResetCompletion();
    setStreakState(next);
    return next;
  }
